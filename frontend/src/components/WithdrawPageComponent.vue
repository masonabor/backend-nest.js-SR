    ...
    const submitWithdraw = async () => {
      ...
      try {
        await axios.put(
          ...
          { accountId: selectedAccount.value, amount: amount.value },
          {
            headers: {
              Authorization: `Bearer ${token}`
            }
        ...
        successMessage.value = `Successfully withdrew ${amount.value} from account`;
        errorMessage.value = '';
        amount.value = 0;
      } catch (error) {
        errorMessage.value = error.response?.data?.message || 'Failed to withdraw';
